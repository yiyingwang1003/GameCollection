...
public class G {
    ...
    public static abstract class Button {
        ...
        public void show(Graphics g) {
            ...
            vs.fill(g, LookAndFeel.back);
            if (boarder) {
                vs.draw(g, enable ? LookAndFeel.boarder : LookAndFeel.disable);
            }
            g.setColor(enable ? LookAndFeel.text : LookAndFeel.disable);
            g.drawString(text, vs.loc.x + LookAndFeel.margin.x, vs.loc.y + dyText);
        }

        public void setSize(Graphics g) {
            FontMetrics fn = g.getFontMetrics();
            vs.size.set(2 * LookAndFeel.margin.x + fn.stringWidth(text), 2 * LookAndFeel.margin.y + fn.getHeight());
            dyText = LookAndFeel.margin.y + fn.getAscent();
        }
